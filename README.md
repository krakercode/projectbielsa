...
`scripts/lua/dump_squad.lua` reads a whole squad rather than just the selection: the senior squad turns out to be a `std::vector<Player*>`, and reading it via its header gives the exact squad — **verified live at 23/23 players, every record complete**. The vector is stable across UI navigation, but nothing points at it, so its address still has to be found once per game launch; a static pointer path is the next job. Staff reads are verified too (all 83 fields against a real coach).

Club state is **not** verified — `pCurrentClub` never populated — and competition/fixture state isn't covered at all, so **Phase 1's exit criterion is not met.** See [docs/phase1-notes.md](docs/phase1-notes.md) for the structure, the tooling, and what's next.
