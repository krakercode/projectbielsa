...
`scripts/lua/dump_squad.lua` reads a whole squad rather than just the selection: the senior squad turns out to be a `std::vector<Player*>`, and the script locates it from the live selection each call so nothing is hardcoded to a heap address. Verified live at 20 players with correct data — but the squad is 23, so it's not exactly right yet, and club/competition/fixture state isn't covered. **Phase 1's exit criterion is not met.** See [docs/phase1-notes.md](docs/phase1-notes.md) for the structure, the tooling, and the known fix.
